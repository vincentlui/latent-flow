...
from nfsde.util import MetricsCallback, SavePlotCallback, dotdict, ResumeTrainingCallback
...
# Flow model args
parser.add_argument('--flow-model', type=str, default='coupling', help='Model name', choices=['coupling', 'resnet', 'gru', 'ode'])
...
parser.add_argument('--invertible', type=bool, default=True, action=argparse.BooleanOptionalAction, help='Invertible')

#Posterior args
parser.add_argument('--posterior-model', type=str, default='CDE', help='posterior encoder model')
...
parser.add_argument('--d-weight-clip', type=float, default=1., help='Discriminator weight clip factor')

# CLPF args
parser.add_argument('--dt', type=float, default=1e-2, help='dt')
parser.add_argument('--time-embedding-dim', type=int, default=3,  help='update generator every n iterations')
parser.add_argument('--noise-type', type=str, default='general', help='noise type of latent sde')
parser.add_argument('--variance-act', type=str, default='softplus', help='activation of latent sde variance')
parser.add_argument('--method', type=str, default='reversible_heun', help='activation of latent sde variance')
parser.add_argument('--rtol', type=float, default=1e-2, help='dt')
parser.add_argument('--atol', type=float, default=1e-2, help='dt')
parser.add_argument("--dt_min", type=float, default=1e-3)
parser.add_argument("--dt_test", type=float, default=1e-3)
parser.add_argument("--dt_min_test", type=float, default=1e-3)
parser.add_argument('--adaptive', type=bool, default=False, action=argparse.BooleanOptionalAction, help='Adaptive')



# Base SDE args
parser.add_argument('--base-sde', type=str, default='ou', help='Base SDE', choices=['brownian', 'ou', 'combine'])
...
# Logs
parser.add_argument('--checkpoint-path', type=str, help='checkpoint path', default=None)
...
if __name__ == '__main__':
    ...
    if args.model in ['flow-gan', 'sde-gan']:
        ...
        trainer = pl.Trainer(
            ...
            callbacks=callbacks,
            default_root_dir="result/" +args.model + "/" + args.data + "/" + datetime.now().strftime('%Y%m%d-%H%M%S'),
            deterministic=True
        )
        ...
        if args.checkpoint_path is not None:
            trainer = pl.Trainer(
                ...
                callbacks=callbacks,
                default_root_dir="result/" + args.model + "/" + args.data + "/" + datetime.now().strftime(
                    '%Y%m%d-%H%M%S'),
